...
type, and joining it with any type SHALL yield that type. It has two sources: the empty value `{}`,
whose type is the empty type — the bottom item type with the zero-or-one occurrence, rendered `{}`,
as `occurrence-types` defines — and which is usable at every `?` and `*` site for exactly that
reason; and a component type parameter that a use site leaves unspecified, which is checked at
that element as the bottom type so that a site binding nothing typed by the parameter needs no
argument. There is no `null` type: the bottom type under an occurrence is the whole of how the
language types an absent value.
...
The system SHALL render it as `never` in a diagnostic that names a type it inferred, and under an
occurrence as `never` with that suffix. Where a diagnostic reports the type of a value the author
wrote as `{}`, or of a value that is the empty value, it SHALL spell it `{}` rather than naming the
...
naming the bottom type or spelling `never*`, for the same reason.

No value SHALL have the bottom type, so it SHALL NOT appear in any runtime representation, in a
value crossing the host boundary, or in a runtime type test. The empty value is a value of the
empty type only statically; at runtime it is the empty sequence.
...
- **WHEN** type inference analyzes the empty braced value `{}`
- **THEN** its type SHALL be the empty type, rendered `{}`
- **AND** it SHALL satisfy `string?`, `int*`, and any other `?` or `*` type, without an expected
  type having been supplied to determine an item type
- **AND** it SHALL NOT satisfy `string` or `string+`
