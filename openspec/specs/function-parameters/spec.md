...
## Purpose
Defines how a `let` function is called: a paren-style function by position or as an element, an
element-style function only as an element, the order a paren-style signature puts the parameters a
caller may omit in, what a call may leave out, and the function evaluating its own defaults.
## Requirements
