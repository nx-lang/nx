# component-type-parameters Specification

## Purpose

Lets a component declare type parameters with the same property syntax as its props, so a
collection-drawing component can say that its items and its template agree on one item type, and
lets a use site supply that type by name or leave it unspecified.
...
carry a default value, MUST NOT carry a modifier such as `content`, MUST NOT carry a `?` or `[]`
suffix on `type`, MUST NOT take the name of a primitive type or of the built-in `Element` type,
and MUST have a name distinct from every other type parameter and prop of the component. The system SHALL reject a violation of any of these rules with a diagnostic that names
the offending definition. A type parameter is a type in the component's props, state, and body
only: an emitted action declared inline in the signature is an action record of its own, checked
and generated outside the component, and a payload field typed by a type parameter SHALL be
...
- **WHEN** a file contains `component <First TItem:type items:TItem[] /> = { state { first:TItem? = null } <Label text="ok" /> }`
- **THEN** analysis SHALL accept the annotation `TItem?` inside the body
...
- **WHEN** a file contains `component <Bad TItem:type? /> = { <Label /> }` and `component <Worse content TItem:type /> = { <Label /> }`
- **THEN** parsing or analysis SHALL reject both definitions
...
compose with the `[]` and `?` suffixes like any other type. Two type parameters, whether of one
...
- **WHEN** a file contains `component <Ok TItem:type items:TItem[] = {} /> = { state { o:object = {items} } <Label /> }` and `let v = <Ok items={} />`
- **THEN** analysis SHALL accept the default `{}` at `TItem[]`, the default `{items}` at `object`, and the binding `items={}`
...
- **AND** SHALL accept `items={"a"}` because the argument `string` is what `TItem[]` expects
...
scope there — and SHALL NOT consult value bindings. A type parameter binding MUST be a plain,
...
rejected. The effective prop types of the component SHALL then be checked with every occurrence
of the parameter replaced by the argument.
...
`Name=` form, rather than reporting a mismatch against `never`.
...
- **AND** `v` is evaluated
- **THEN** the resulting external component record SHALL have an `itemsSource` field and SHALL NOT have a `TItem` field
