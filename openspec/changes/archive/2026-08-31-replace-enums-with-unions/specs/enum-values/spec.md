...
**Reason**: The `enum` declaration form no longer exists. The behavior it specified is unchanged and
is restated for constant union cases by *Constant case names are preserved exactly across runtime
and tooling*, which keeps the bare-authored-string wire contract and the split between source-position
and display rendering.
**Migration**: Write `type DealStage = draft | pending_review | closed_won` instead of
`enum DealStage = ...`. No wire shape, payload, or host contract changes for such a declaration.

### Requirement: Enum members are referenceable without naming the enum type
**Reason**: Enum members and payloadless union cases are one concept. Restated as *Constant cases
are referenceable without naming the union type*, with the same resolution rules and the same
interim reporting of a needed import.
**Migration**: None for authors; a bare name at a union-typed site resolves exactly as it did at an
enum-typed site.

### Requirement: First-party enum examples and fixtures use snake_case by convention
**Reason**: Restated for constant union cases by *First-party constant-union examples and fixtures
use snake_case by convention*. The convention itself is unchanged.
**Migration**: None. Existing `snake_case` member names carry over as case names verbatim.
