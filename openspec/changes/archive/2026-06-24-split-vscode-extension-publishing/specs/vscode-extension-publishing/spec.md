...
production path SHALL be a dedicated `Publish VS Code extension` workflow that consumes artifacts
from a successful `VS Code Extension` workflow run on `main`; manual publishing MAY remain available
as a repair path for an already-built artifact run.
...
#### Scenario: Publish workflow uses a successful VS Code Extension source run

- **WHEN** the `Publish VS Code extension` workflow is triggered automatically after a source
  workflow run
- **THEN** the source run SHALL be a successful `VS Code Extension` workflow run from `main` in the
  NX repository
- **AND** the publish workflow SHALL download the verified VSIX artifacts from that source run
- **AND** the publish workflow SHALL publish those artifacts without rebuilding different package
  contents
...
#### Scenario: Manual repair uses an explicit artifact run

- **WHEN** a maintainer runs a manual repair publish for the VS Code extension
- **THEN** the workflow SHALL require an explicit artifact-producing workflow run ID
- **AND** the workflow SHALL validate that source run before publication
- **AND** the workflow SHALL publish artifacts from that source run without rebuilding different
  package contents
...
only in the dedicated `Publish VS Code extension` workflow after extension tests, package
verification, version checks, and required environment gates pass.
...
- **WHEN** the `Publish VS Code extension` workflow targets the `production` environment for a
  successful trusted `main` source run
- **AND** extension tests pass in the source workflow
...
- **AND** any production Marketplace or Open VSX writes SHALL happen in the separate
...
- **AND** it SHALL describe how CI packages verified VSIX artifacts from trusted `main` builds
- **AND** it SHALL describe how the separate `Publish VS Code extension` workflow publishes those
  verified artifacts
