...
- **WHEN** a `LibraryRegistry` contains loaded `../question-flow` and `../internal-admin` snapshots
...
- **THEN** program construction SHALL fail with a library-load diagnostic for the missing normalized
  root
...
- **THEN** program construction SHALL return a diagnostic that names the discovered dependency chain
...
been built successfully, runtime evaluation and component lifecycle calls SHALL execute against that
`ProgramArtifact` without requiring the build context to remain alive.
