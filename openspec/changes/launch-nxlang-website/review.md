...
### ✅ Verified - RF1 On the landing page and the 404 page, the desktop header shows its links twice and wraps onto a second row.
- **Severity:** High
...
- **Verification:** Confirmed. `.narrow-links` is hidden by `display: none` in the component's own `@media (min-width: 50rem)`, which follows and so overrides the unlayered `display: flex`. Rebuilt and served under `wrangler@4.141.0 dev`, Playwright finds the header links `NX, Docs, Playground, GitHub` exactly once on `/`, an unknown path and `/language-tour/types/` at 1280 px and 800 px, all in a 64 px header. At 375 px they show on the sidebar-less pages and sit in the mobile menu on the docs page. A 1280 px screenshot of `/` shows one row: logo, search, links, GitHub, theme select.

### ✅ Verified - RF2 The playground's deploy no longer runs when the WASI sysroot pin changes, because `scripts/**` was dropped from its paths.
- **Severity:** Medium
...
- **Verification:** Confirmed. `deploy-playground.yml:21-22` lists `scripts/fetch-wasi-sysroot.mjs` with a comment. It is the only root script `build-wasm.mjs` imports, so the narrower path is enough.

### ✅ Verified - RF3 The playground's cache policy has no automated test now that the server tests are gone.
- **Severity:** Low
...
- **Verification:** Confirmed. `sites/playground/headers.test.mjs` is in the `test` script and its 3 tests pass. Renaming the `/playground/shaders/*` rule to `/playground/shader/*` made it fail with `public/shaders has a rule`, and `_headers` was restored afterwards. The optional smoke-test assertion was not added, which is fine.

### ✅ Verified - RF4 The website's content-hashed assets are revalidated on every page load.
- **Severity:** Low
...
- **Verification:** Confirmed. `sites/website/public/_headers` reaches `dist/_headers`. Under `wrangler dev`, `/_astro/*.css` answers `public, max-age=31536000, immutable`, `/` keeps `max-age=0, must-revalidate`, and `/_headers` answers 404.

### ✅ Verified - RF5 The not-found page is included in `llms-full.txt` and `llms-small.txt`.
- **Severity:** Low
...
- **Verification:** Confirmed. After a rebuild, `Page not found` appears 0 times in `llms-full.txt` and `llms-small.txt`. `dist/404.html` carries the header and the `nx-build` stamp and has no `data-pagefind-body`, and the sitemap has no 404 entry. `wrangler dev` answers `/nope` with 404 and that page. The build's earlier warning that `/404` conflicts with `/[...slug]` is gone too. The website test still passes (168 blocks, now in 22 files).

### ✅ Verified - RF6 The change's artifacts no longer match what was built.
- **Severity:** Low
- **Evidence:**
...
- **Verification:** Confirmed. proposal.md, design.md and task 6.2 name the real `_headers` paths, and design.md says why. The paths sentence matches the workflow, the fiddle decision is recorded in Content work, the Open Questions section is gone, and task 4.3's verify clause reads whole. `openspec validate launch-nxlang-website --strict` passes. One dangling reference is left: finished task 5.1 still says "(design.md's open question)", which is harmless.
...
- `openspec/changes/archive/2026-09-25-update-fiddle-to-nx-occurrences/tasks.md` is staged with 8.1–8.6 checked
  ...
  - *Answered:* it was committed on its own (`25f5030`), apart from this change's commit.
- `docs/README.md` is untracked (`??`) while its old content is staged as the rename to `sites/website/README.md`,
  ...
  - *Answered:* `docs/README.md` is tracked in `782b175`.
