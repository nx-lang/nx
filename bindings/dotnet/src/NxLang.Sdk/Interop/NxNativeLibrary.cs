...
{
    internal const uint SupportedAbiVersion = 12;
    ...
    private static InvalidOperationException CreateAbiVersionException(uint actualAbiVersion)
    {
        return new InvalidOperationException(
            $"NX native runtime ABI mismatch. Managed binding expects ABI {SupportedAbiVersion}, but loaded ABI {actualAbiVersion}. Rebuild `crates/nx-ffi` from the same NX source revision as `NxLang.Sdk.dll`.");
    }

    private static InvalidOperationException CreateMissingLibraryException(DllNotFoundException innerException)
    {
        return new InvalidOperationException(
            "NX native runtime could not be found. Package consumers should restore and publish for a supported runtime identifier so the `NxLang.Sdk` package stages its `nx_ffi` native asset. Source consumers should build `crates/nx-ffi` and import `bindings/dotnet/build/NxLang.Sdk.targets` from the same NX checkout.",
            innerException);
    }
    ...
    {
        return new InvalidOperationException(
            "NX native runtime could not be loaded. Ensure the staged native library matches the current platform and comes from the same NX source revision as `NxLang.Sdk.dll`.",
            innerException);
    }
