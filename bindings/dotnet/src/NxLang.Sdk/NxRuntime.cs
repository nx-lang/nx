...
/// <summary>
/// Provides convenience methods for validating, building, generating IR from, and evaluating NX source through the
/// native NX SDK.
/// </summary>
public static class NxRuntime
{
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code and returns bytes in the canonical MessagePack wire
    /// format.
    /// </summary>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    /// <param name="fileName">Optional file name used for diagnostics and local import normalization.</param>
    /// <returns>The evaluation result serialized as canonical NX MessagePack bytes.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="source"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="NxEvaluationException">Thrown when build or evaluation reports NX diagnostics.</exception>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the native runtime returns an invalid payload.
    /// </exception>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code and returns bytes in the requested output format.
    /// </summary>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    /// <param name="outputFormat">Output format requested from the native NX runtime.</param>
    /// <param name="fileName">Optional file name used for diagnostics and local import normalization.</param>
    /// <returns>The evaluation result serialized in the requested output format.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="source"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="NxEvaluationException">Thrown when build or evaluation reports NX diagnostics.</exception>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code against a caller-supplied build context.
    /// </summary>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code against a caller-supplied build context in the
    ...
    /// </summary>
    /// <param name="workspace">Workspace containing source modules to validate.</param>
    ...
    /// </exception>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the native runtime cannot validate the workspace.
    /// </exception>
    ...
    /// </summary>
    /// <param name="programArtifact">Program artifact to evaluate.</param>
    /// <returns>The evaluation result serialized as canonical NX MessagePack bytes.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="programArtifact"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="NxEvaluationException">Thrown when evaluation reports NX diagnostics.</exception>
    /// <exception cref="ObjectDisposedException">
    /// Thrown when <paramref name="programArtifact"/> has already been disposed.
    /// </exception>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code and returns the JSON result.
    /// </summary>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    /// <param name="fileName">Optional file name used for diagnostics and local import normalization.</param>
    /// <returns>The normalized JSON-compatible NX value as a <see cref="JsonElement"/>.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="source"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="NxEvaluationException">Thrown when build or evaluation reports NX diagnostics.</exception>
    /// <exception cref="InvalidOperationException">Thrown when the native runtime returns invalid JSON.</exception>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code against a caller-supplied build context and returns the
    /// JSON result.
    /// </summary>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    /// <param name="buildContext">Build context used to resolve preloaded libraries.</param>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code and deserializes the MessagePack result.
    /// </summary>
    /// <typeparam name="T">The CLR type to deserialize from the NX MessagePack payload.</typeparam>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    /// <param name="fileName">Optional file name used for diagnostics and local import normalization.</param>
    /// <returns>The deserialized evaluation result.</returns>
    /// <exception cref="ArgumentNullException">
    /// Thrown when <paramref name="source"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="NxEvaluationException">Thrown when build or evaluation reports NX diagnostics.</exception>
    /// <exception cref="MessagePackSerializationException">
    /// Thrown when the result cannot be deserialized as <typeparamref name="T"/>.
    /// </exception>
    ...
    /// <summary>
    /// Evaluates the <c>root()</c> entrypoint in NX source code against a caller-supplied build context and
    /// deserializes the MessagePack result.
    /// </summary>
    /// <typeparam name="T">The CLR type to deserialize from the NX MessagePack payload.</typeparam>
    /// <param name="source">The NX source code to evaluate. It must contain a <c>root()</c> function.</param>
    /// <param name="buildContext">Build context used to resolve preloaded libraries.</param>
    ...
    /// Thrown when <paramref name="source"/> is <see langword="null"/>.
    /// </exception>
    /// <exception cref="NxEvaluationException">Thrown when build or IR generation reports NX diagnostics.</exception>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the native runtime returns invalid IR metadata.
    /// </exception>
    ...
    /// <summary>
    /// Evaluates a named component from a program artifact using no explicit props or state and returns the JSON
    /// result.
    /// </summary>
    public static JsonElement EvaluateComponentJson(
        NxProgramArtifact programArtifact,
    ...
    /// <summary>
    /// Initializes a named component from a program artifact using no explicit props and deserializes the rendered
    /// result.
    /// </summary>
    public static NxComponentInitResult<TElement> InitializeComponent<TElement>(
        NxProgramArtifact programArtifact,
    ...
    /// <summary>
    /// Initializes a named component from a program artifact with MessagePack-serializable props and deserializes the
    /// result.
    /// </summary>
    public static NxComponentInitResult<TElement> InitializeComponent<TProps, TElement>(
        NxProgramArtifact programArtifact,
