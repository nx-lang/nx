...
cannot extend a union to add cases. The names in a case list are always new cases, never existing
types: `type Result = Success | Failure` declares a union with the cases `Result.Success` and
`Result.Failure`, even where records named `Success` and `Failure` are in scope.
...
```nx fragment
// User and UserBase as declared under Record Types above.
let user =
  <User
    id="123"
    name="John Doe"
...
An `int64` bound and a floating-point bound have no common type, so the range is rejected:
