...
let <UserForm user:User isLocked:boolean /> =
  <form>
...
```nx
let <DialogButton primary:boolean /> =
  <Button
    if primary {
      label="Save"
      tone="strong"
...
```nx
let <StatusBadge isError:boolean isWarning:boolean /> =
  <Badge
    if {
      isError => tone="danger"
...
let <StatusNotice state:LoadState /> =
  <Notice
    if state is {
...
```nx
type Check = <function value:string />: boolean
type Rule = { field:string check:Check }

let notBlank(value:string): boolean = { value != "" }

let <SignupForm isLoading:boolean hasErrors:boolean /> =
  <Form
    rules={ <Rule field="email" check={notBlank}/> <Rule field="name" check={notBlank}/> }
...
Expressions, conditionals, sequences, and function values are valid attribute values. A function is
passed by name, as a [function value](/reference/syntax/functions#functions-as-values).
...
let card = <UserCard user=<User id="456" name="Jane" email="jane@example.com"/> />
let canvas = <DrawingCanvas points={ <Point x=10 y=20/> <Point x=30 y=40/> } />
```
