...
function readRegistryTags() {
  const file = join(appRoot, "src/drawnui/react/registry.ts");
  ...
  visit(source);
  if (tags.length === 0) {
    throw new Error("Could not read Registry from the vendored registry.");
  }
  return tags;
}
...
// rather than restated here, and the catalog cannot drift from the vendored code it was generated
// against.
`;
