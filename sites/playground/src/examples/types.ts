...
export function coverageNote(example: Example): string | null {
  ...
  switch (example.coverage) {
    case "complete":
      return null;
    case "static":
      return `Static: drawn as in the original, but nothing here responds. NX has no ${list} yet.`;
    case "reduced":
      return `Reduced: the original demonstrates ${example.demonstrates ?? "more than this"}. That needs ${list}, which NX does not have yet.`;
  }
}
