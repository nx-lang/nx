...
## Property-List Conditional And Match Fragments

The grammar has syntax for conditional property lists, including match-style
forms such as `<View if state is { LoadState.failed => message=state.message } />`,
but the HIR currently models element properties as a flat `Vec<Property>`.
That shape is not rich enough to represent branches that produce zero or more
property bindings, and lowering property-list matches into ordinary
`Expr::Match` would blur two different result domains: scalar/value
expressions versus property fragments.

If this is revisited in the future:
- Introduce a first-class property-fragment HIR, such as `PropertyEntry::Value`,
  `PropertyEntry::If`, and `PropertyEntry::Match`, instead of trying to encode
  conditional property lists as value expressions.
- Apply the same narrowing and exhaustiveness rules used by value match
  expressions inside each property-match arm, so union case properties can be
  read safely after a matching pattern.
- Define required-property analysis across branches: a required prop should be
  considered supplied only when every reachable branch supplies it.
- Define duplicate-property analysis path-sensitively: the same prop in
  mutually exclusive branches should be allowed, while duplicates on the same
  possible path should be rejected.
- Treat ambiguous static-plus-conditional duplicate props as errors rather than
  using source order or "last wins" semantics.
- Until this is implemented end to end, lowering should emit a clear
  unsupported-feature diagnostic for property-list conditionals or matches
