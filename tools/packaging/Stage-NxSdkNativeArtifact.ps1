...
$libraryName = Get-NxSdkNativeLibraryName -RuntimeIdentifier $RuntimeIdentifier
Assert-NxSdkHostMatchesRuntimeIdentifier -RuntimeIdentifier $RuntimeIdentifier

if (!$OutputRoot) {
    $OutputRoot = "$RepoRoot/artifacts/nx-sdk"
}
...
if (!(Test-Path $sourcePath)) {
    throw "Expected NX native SDK library at '$sourcePath'."
}
...
Write-Host "Staged $RuntimeIdentifier native SDK asset: $destinationPath"
