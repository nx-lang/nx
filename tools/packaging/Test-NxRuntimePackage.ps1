...
try {
    ...
    if ($metadata.description -notmatch 'Managed \.NET bindings' -or $metadata.description -notmatch 'native nx_ffi runtime assets') {
        throw "Package '$PackagePath' description must describe the managed .NET binding and packaged native NX runtime assets."
    }
} finally {
    $archive.Dispose()
}
