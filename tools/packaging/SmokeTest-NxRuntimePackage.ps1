...
$workRoot = Join-Path ([System.IO.Path]::GetTempPath()) "nx-runtime-smoke-$([Guid]::NewGuid().ToString('N'))"
...
try {
    ...
    $packageVersion = [regex]::Match($package.Name, '^NxLang\.Runtime\.(?<version>.+)\.nupkg$').Groups['version'].Value
    if (!$packageVersion) {
        throw "Could not infer package version from '$($package.Name)'."
    }

    dotnet add $appRoot package NxLang.Runtime --version $packageVersion --source $feedRoot | Out-Host
    if ($LASTEXITCODE -ne 0) {
...
Write-Host "Verified NX runtime package consumption for $RuntimeIdentifier."
