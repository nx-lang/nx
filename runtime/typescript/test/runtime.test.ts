...
class ArtifactBuilder {
   ...
   */
  functionType(result: number, params: readonly (readonly [string, number, number?])[] = []): number {
    return this.type([
      typeKinds.function,
      result,
      params.length,
  ...
  text(operand: number, type: string): number {
  ...
  fn(
    name: string,
    body: number,
    params: readonly Cells[] = [],
    options: { result?: number; optionalResult?: boolean } = {},
  ): number {
  ...
  component(
    name: string,
    props: readonly Cells[],
    state: readonly Cells[],
    body: number,
    options: { external?: boolean; abstract?: boolean; emits?: readonly (readonly [string, Cells])[] } = {},
  ): number {
  ...
  handler(component: Cells, emit: string, action: Cells, slot: number, owner: Cells | undefined, body: number): number {
...
function textOfParameter(type: string): NxPreparedProgram {
  const b = new ArtifactBuilder("main.nx");
 ...
 */
function counterArtifact(): Uint8Array {
  const b = new ArtifactBuilder("main.nx");
  const string = b.primitive("string");
  const int = b.primitive("int");
  ...
  const button = (label: string, handler: number): number =>
    b.node([nodeKinds.component, ...b.ref("Button"), ...b.list([b.property("label", b.string(label)), b.property("onTapped", handler)]), ...b.content([])]);
  ...
  const counter = (property: string, handler: number): number =>
    b.node([nodeKinds.component, ...b.ref("Counter"), ...b.list([b.property(property, handler), b.property("step", b.int(2))]), ...b.content([])]);
...
function counterToken(rendered: NxCanonicalValue, index = 0): string {
  const button = fields((rendered as NxCanonicalValue[])[index]!);
...
function tapped(token: string): NxCanonicalValue {
...
test("a descriptor keeps its handler property as a token-less ActionHandler record", () => {
  ...
  assertEqual(fields((evaluated as NxCanonicalValue[])[0]!).onTapped, { $type: "ActionHandler", action: "Button.Tapped" });
...
test("a handler for a same-named component of another module is refused naming both modules", () => {
  ...
  const refused = assertThrows(
    () => evaluateFunction(program, "root"),
    "Expected Button props.onTapped to be an action handler for ui/button.nx::Button.Tapped, got one for main.nx::Button.Tapped.",
  );
...
test("dispatch refuses the wrong action, an action the component does not emit, and a malformed entry", () => {
  ...
  const wrong = assertThrows(
    () => dispatchComponentActions(program, instance, [{ $type: "ActionHandlerInvocation", token: "h1-1", action: { $type: "Slider.EndChanged" } }]),
    "Expected an action of type 'Button.Tapped' for handler Button.Tapped, got 'Slider.EndChanged'",
  );
 ...
 */
function handlersProgram(): NxPreparedProgram {
...
function children(instance: { rendered: NxCanonicalValue }): Record<string, NxCanonicalValue>[] {
  return (fields(instance.rendered).children as NxCanonicalValue[]).map(fields);
}

/** The rendered descriptors of `type` in a list, in order. */
function ofType(list: readonly NxCanonicalValue[], type: string): Record<string, NxCanonicalValue>[] {
  return list.map(fields).filter((child) => child.$type === type);
}

function searchBoxProps(descriptor: Record<string, NxCanonicalValue>): Record<string, NxCanonicalValue> {
...
test("a handler in a content child stays the parent's and is unowned by the child that renders it", () => {
  ...
  const [button] = ofType(fields(stack.rendered).content as NxCanonicalValue[], "Button");
...
test("an ActionHandler record in props needs a parent instance and a token that instance holds", () => {
  ...
  const stale = assertThrows(
    () => initializeComponent(program, "SearchBox", { onSearchSubmitted: { $type: "ActionHandler", action: "SearchSubmitted", token: "h7-1" } }, { parent: page.instance }),
    "Unknown handler token 'h7-1'",
  );
  assertEqual(stale.diagnostics[0]!.code, "nx-ir-handler-token");
  // A handler for another component or emit is refused where it is bound.
  const [button] = ofType(children(page), "Button");
  assertThrows(
    () => initializeComponent(program, "SearchBox", { onSearchSubmitted: button!.onTapped! }, { parent: page.instance }),
    "Expected SearchBox props.onSearchSubmitted to be an action handler for SearchBox.SearchSubmitted, got one for Button.Tapped",
  );
  const instance: NxComponentInstance = page.instance;
...
test("a handler resolved through the parent is the parent's own value", () => {
  ...
  const page = initializeComponent(program, "Page");
  const [button] = ofType(children(page), "Button");
  const pageToken = fields(button!.onTapped!).token as string;
  const stack = initializeComponent(program, "Stack", { children: fields(page.rendered).children! }, { parent: page.instance });
  const [rendered] = ofType(fields(stack.rendered).content as NxCanonicalValue[], "Button");
  const stackToken = fields(rendered!.onTapped!).token as string;
...
function recordNode(b: ArtifactBuilder, name: string, properties: readonly (readonly [string, number])[]): number {
...
function emptyNode(b: ArtifactBuilder): number {
 ...
 */
function occurrencesArtifact(): Uint8Array {
  const b = new ArtifactBuilder("main.nx");
  const string = b.primitive("string");
  const int = b.primitive("int");
  ...
  const book: Cells = [b.str("b"), b.nominal("Book"), 0];
  const author = (): number => b.node([nodeKinds.member, b.node([nodeKinds.slot, 0, b.str("b")]), b.str("author")]);
  ...
  const o: Cells = [b.str("o"), b.optional(int), 2];
  const slotO = (): number => b.node([nodeKinds.slot, 0, b.str("o")]);
...
test("reads a seq type and refuses the retired array and nullable kinds, the null node and a malformed seq", () => {
  ...
  const refused = (build: (builder: ArtifactBuilder) => void, message: string): void => {
...
test("an empty optional field is an omitted key, written or not, and reads as the empty value", () => {
  ...
  const string = b.primitive("string");
  b.record("Person", [b.field("name", string, { required: true }), b.field("nick", b.optional(string))]);
  const person = (nick?: number): number => recordNode(b, "Person", [["name", b.string("Ada")], ...(nick === undefined ? [] : [["nick", nick] as const])]);
  b.fn("omitted", person());
  b.fn("writtenEmpty", person(emptyNode(b)));
  b.fn("written", person(b.string("A")));
  const p: Cells = [b.str("p"), b.nominal("Person"), 0];
...
test("an evaluated update record clears only a clearable field, and apply, diff and changed carry the clearing", () => {
  ...
  b.record("User", [b.field("name", string, { required: true }), b.field("email", b.optional(string))]);
  b.record("User.Update", [b.field("name", string), b.field("email", b.optional(string))], { updateTarget: b.ref("User") });
  const ada = (): number => recordNode(b, "User", [["name", b.string("Ada")], ["email", b.string("a@b")]]);
  const intrinsic = (op: number, args: readonly number[], order: readonly string[] = []): number =>
    b.node([nodeKinds.intrinsic, op, ...b.content(args), ...b.list(order.map((name) => [b.str(name)]))]);
...
test("a lone content child binds by the declared occurrence", () => {
  ...
  const child = (): number => recordNode(b, "Item", [["n", b.int(1)]]);
  const use = (name: string, children: readonly number[]): number => b.node([nodeKinds.component, ...b.ref(name), ...b.list([]), ...b.content(children)]);
...
test("a declared result normalizes the body's value, and an empty optional result reaches the host as null", () => {
  const b = new ArtifactBuilder("main.nx");
  const int = b.primitive("int");
  const untaken = (): number => b.node([nodeKinds.if, b.node([nodeKinds.bool, 0]), b.int(1), NX_IR_NONE]);
