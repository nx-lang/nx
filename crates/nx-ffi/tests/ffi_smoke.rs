...
use nx_ffi::{
    ...
    nx_component_dispatch_result_msgpack_to_json, nx_component_init,
    nx_component_init_program_artifact, nx_component_init_result_msgpack_to_json,
    nx_diagnostics_msgpack_to_json, nx_eval_program_artifact, nx_eval_source, nx_ffi_abi_version,
    nx_free_buffer, nx_free_program_artifact, nx_value_msgpack_to_json, NxBuffer, NxEvalStatus,
    NxProgramArtifactHandle, NX_FFI_ABI_VERSION,
};
...
#[test]
fn ffi_program_artifact_handle_reuses_library_items_during_eval() {
    ...
    std::fs::write(
        ...
        r#"
            let <QuestionFlowTemplate title:string /> =
              <div>{title}</div>
        "#,
    )
    .expect("library file");
...
let root() = { <QuestionFlowTemplate title="Welcome" /> }"#;
    std::fs::write(&main_path, source).expect("main file");

    let (handle, status, bytes) =
        build_program_artifact_handle(source, &main_path.display().to_string());
    assert!(matches!(status, NxEvalStatus::Ok));
    assert!(bytes.is_empty());
    assert!(!handle.is_null());

    let (eval_status, payload) = eval_msgpack_with_program_artifact(handle);
    nx_free_program_artifact(handle);

    assert!(matches!(eval_status, NxEvalStatus::Ok));
    let value = NxValue::from_msgpack_slice(&payload).unwrap();
    let NxValue::Record { type_name, .. } = value else {
        panic!("Expected rendered element record");
    };
    assert_eq!(type_name.as_deref(), Some("div"));
}

#[test]
fn ffi_component_init_with_program_artifact_reuses_library_component() {
    let temp = TempDir::new().expect("temp dir");
    let app_root = temp.path().join("app");
    let library_root = temp.path().join("question-flow");
    std::fs::create_dir_all(&app_root).expect("app root");
...
let root() = { 0 }"#;
    ...
    let (init_status, payload) =
        component_init_msgpack_with_program_artifact(handle, "SearchBox", Some(&props_msgpack));
    nx_free_program_artifact(handle);

    assert!(matches!(init_status, NxEvalStatus::Ok));
    let init_result: ComponentInitResult = rmp_serde::from_slice(&payload).unwrap();
...
#[test]
fn ffi_component_dispatch_with_program_artifact_reuses_library_component() {
    let temp = TempDir::new().expect("temp dir");
    let app_root = temp.path().join("app");
    let library_root = temp.path().join("question-flow");
    std::fs::create_dir_all(&app_root).expect("app root");
...
let root() = { 0 }"#;
    ...
    let (init_status, init_payload) =
        component_init_msgpack_with_program_artifact(handle, "SearchBox", None);
    assert!(matches!(init_status, NxEvalStatus::Ok));
    ...
    let (dispatch_status, dispatch_payload) = component_dispatch_msgpack_with_program_artifact(
        handle,
        &init_result.state_snapshot,
        &actions_msgpack,
    );
    nx_free_program_artifact(handle);

    assert!(matches!(dispatch_status, NxEvalStatus::Ok));
    let dispatch_result: ComponentDispatchResult =
        rmp_serde::from_slice(&dispatch_payload).unwrap();
...
fn ffi_component_dispatch_round_trips_effect_payloads_in_msgpack_and_debug_json() {
    ...
    let program = load_program_artifact_from_source(source, "ffi-dispatch.nx")
    ...
    let (handle, status, bytes) = build_program_artifact_handle(source, "ffi-dispatch.nx");
    assert!(matches!(status, NxEvalStatus::Ok));
    assert!(bytes.is_empty());
    assert!(!handle.is_null());
